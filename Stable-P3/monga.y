...
idList:
	TK_ID					{ $$ = ABS_id($1.name); }
	| idList ',' TK_ID		{ $$ = ABS_mergeList($1, ABS_id($3.name)); }
	;		
...
variable:
	TK_ID							{ $$ = ABS_varMono( ABS_id($1.name) ); } 
...
cmdCall:
	TK_ID '(' expressionList ')'			{ $$ = ABS_expCall( ABS_id($1.name) , $3); }
	;
...
parameter:
	type TK_ID			{ $$ = ABS_declVar($1, ABS_id($2.name)); }
	;
...
functionDeclaration:
	type TK_ID '(' parameterList ')' block 					{ $$ = ABS_declFunc($1, ABS_id($2.name), $4, $6); } 
	| TK_VOID TK_ID '(' parameterList ')' block				{ $$ = ABS_declFunc( TYP_getID( VOID ) , ABS_id($2.name), $4, $6); } 
	;
